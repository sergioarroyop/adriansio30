...
const player = {
  ...
  h: 30,              // alto - tamaño apropiado para 1000px
...
function createFloors(){
  ...
  for(let i=0;i<TOTAL_FLOORS;i++){
    ...
    if (i === 0) {
      ...
      enemies.push({ x: GAME_WIDTH/2, y: y - FLOOR_GAP/2, w: 25, h: 20, vx: 2, floorIndex: i, type: 'flying' });
    } else if (i === 1) {
      // Piso 2: un terrestre aleatorio + dos voladores
      const groundX1 = Math.random() * (GAME_WIDTH - 50) + 25;
      enemies.push({ x: groundX1, y: y - FLOOR_THICKNESS/2 - 25, w: 25, h: 25, vx: Math.random() > 0.5 ? 2 : -2, floorIndex: i, type: 'ground' });
      enemies.push({ x: GAME_WIDTH/4, y: y - FLOOR_GAP/2, w: 25, h: 20, vx: 2, floorIndex: i, type: 'flying' });
      enemies.push({ x: (3*GAME_WIDTH)/4, y: y - FLOOR_GAP/2, w: 25, h: 20, vx: -2, floorIndex: i, type: 'flying' });
    } else if (i === 2) {
...
function draw(){
  ...
  for (const e of enemies) {
    ...
    if (e.type === 'ground') {
      if (groundEnemyImg.complete) {
        ctx.drawImage(groundEnemyImg, scaledEnemyX, scaledEnemyY, scaledEnemyW, scaledEnemyH);
      } else {
        ctx.fillStyle = 'red';
      ...
      if (flyingEnemyImg.complete) {
        ctx.drawImage(flyingEnemyImg, scaledEnemyX, scaledEnemyY, scaledEnemyW, scaledEnemyH);
      } else {
        ctx.fillStyle = 'blue';
        ctx.fillRect(scaledEnemyX, scaledEnemyY, scaledEnemyW, scaledEnemyH);
      }
  ...
  if (goal) {
    ...
    const scaledGoalW = scaleSize(goal.w);
    const scaledGoalH = scaleSize(goal.h);
    
    // La meta está en el piso 5, posicionarla sobre el suelo visible
    const scaledGoalY = canvasHeight - scaleSize(FLOOR_THICKNESS) - scaledGoalH;
    console.log(`Meta piso 5: scaledY=${scaledGoalY}, canvasHeight=${canvasHeight}`);
